...
func MakeHTTPHandler(endpoints Endpoints, responseEncoder httptransport.EncodeResponseFunc, options ...httptransport.ServerOption) http.Handler {
	...
	m.Methods("GET").Path("/maples/hello").Handler(httptransport.NewServer(
		endpoints.HelloEndpoint,
		DecodeHTTPHelloZeroRequest,
		responseEncoder,
		serverOptions...,
	))

	m.Methods("POST").Path("/maples/user").Handler(httptransport.NewServer(
		endpoints.AddUserEndpoint,
		DecodeHTTPAddUserZeroRequest,
		responseEncoder,
		serverOptions...,
	))
...
// DecodeHTTPHelloZeroRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded hello request from the HTTP request
// body. Primarily useful in a server.
func DecodeHTTPHelloZeroRequest(_ context.Context, r *http.Request) (interface{}, error) {
	defer r.Body.Close()
	...
	if NameHelloStrArr, ok := queryParams["name"]; ok {
		NameHelloStr := NameHelloStrArr[0]
		NameHello := NameHelloStr
		req.Name = NameHello
	}
...
// DecodeHTTPAddUserZeroRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded adduser request from the HTTP request
// body. Primarily useful in a server.
func DecodeHTTPAddUserZeroRequest(_ context.Context, r *http.Request) (interface{}, error) {
	defer r.Body.Close()
...
// DecodeHTTPUpdateUserMessageZeroRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded updateusermessage request from the HTTP request
// body. Primarily useful in a server.
